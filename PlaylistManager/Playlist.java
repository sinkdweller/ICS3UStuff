...
// JE 17
// ==============================================================================
// Problem Definition – PLaylist manager
//==============================================================================
...
static String[][] songArray = new String[10][5];										//Playlist Array
	...
	/* *printTable
	 * This procedural method prints the playlist in table format to a FILE
	 * @return void
	 * @throws IOException
	 */
	public void printTable()throws IOException {
		...
		PrintWriter output; 									
		output = new PrintWriter(new FileWriter("/Users/evelynnlu/eclipse-workspace/Culminating/PlaylistTable")); //print to one file only
		output.println("#index         SONG NAME:                 ARTIST:                    GENRE:                     ALBUM:                     RATING:");	//print the column names
		int index =1;//starting index of song
		for(int i =0; i<songArray.length; i++) { //loop through array
			...
			if(songArray[i][0] != null) {		//if there is an element at the index
				...
				String x;					//What to print at the index space
					x = ""+ index;			//or don't
				output.print(x);//print the index
				index ++;
				for(int Space1 = 0; Space1 <15 - x.length(); Space1++ )	//print calculated spaces - index space
					output.print(" ");//print space
				
				for(int j = 0; j<5; j++) {					//for each element in the array print the element
					output.print(songArray[i][j]);							
					lastWordLength = songArray[i][j].length();					//Last word length is subtracted from usual space # to print consistent spaces
					for(int s =0; s<( 27- lastWordLength); s++) 
						output.print(" ");//print space
	...
	*/
	public void printMenuOptions() {//start of printMenuOptions method
		...
		System.out.println("[8] to print as TABLE to file");
	 ...
	 */
	public boolean checkSort(String arr[][], int sortFactor, boolean ascending) {	//startcheckSort method
		String checkHigher;//word to check higher
		String checkLower;//word to check if lower
		for(int i =0; i<entryIndx-1; i++) {
				if(ascending == true) {
					 checkHigher = arr[i+1][sortFactor].toLowerCase(); //check if later element is higher in ascending
					 checkLower = arr[i][sortFactor].toLowerCase();		//check if earlier element is lower
				}//end of if
				else {
					 checkLower = arr[i+1][sortFactor].toLowerCase();	//check if earlier element is higher in descending
					 checkHigher = arr[i][sortFactor].toLowerCase();	//variable to check if later is lower
				}//end of else
	 ...
	 */
	 public String askForNum(String ask, int num, String error)throws IOException {//start of askForNum method
	 ...
	 * @IOException
	 */
	public void makeNewFile(String name)throws IOException{//start of makeNewFile method
	 ...
	 * @return void
	 */
	public void makeRandomPlaylist(int rows) {	//start of make Random Playlist method
		...
		for(int i =0; i < ranArray.length; i++) {	//fill the Array at each index with random things
			ranArray[i][GENRE] = RANGENRE[(int) Math.round(Math.random()*(RANGENRE.length-1))];			//get random genre
			ranArray[i][SNAME] = RANSNAME[(int) Math.round(Math.random()*(RANSNAME.length-1))];			//get random song name
			ranArray[i][ALBUM] = RANALBUM[(int) Math.round(Math.random()*(RANALBUM.length-1))];			//get random album name
			ranArray[i][ARTIST] = RANARTIST[(int) Math.round(Math.random()*(RANARTIST.length-1))];		//get random artist name
			ranArray[i][RATING] = ""+ ((int) Math.round(Math.random()*5));								//get random rating
